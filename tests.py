...
# Once this is complete, run $ python manage.py test tradebooks.tests
# 
...
class TemplatesStructureTests(TestCase):
    ...
    def setUp(self):
        ...
        self.rango_templates_dir = os.path.join(self.templates_dir, 'tradebooks')
    ...
    def test_tradebooks_templates_directory_exists(self):
        """
        Does the templates/rango/ directory exist?
        """
        directory_exists = os.path.isdir(self.rango_templates_dir)
        self.assertTrue(directory_exists, f"{FAILURE_HEADER}The tradebooks templates directory does not exist.{FAILURE_FOOTER}")
    ...
    def test_templates_exist(self):
        ...
        """
        index_path = os.path.join(self.tradebooks_templates_dir, 'index.html')
        about_path = os.path.join(self.tradebooks_templates_dir, 'about.html')
...
class IndexPageTests(TestCase):
    ...
    Image tests are in the StaticMediaTests suite.
    """
    def setUp(self):
        self.response = self.client.get(reverse('tradebooks:index'))
    
    def test_index_uses_template(self):
        """
        Checks whether the index view uses a template -- and the correct one!
        """
        self.assertTemplateUsed(self.response, 'tradebooks/index.html', f"{FAILURE_HEADER}Your index() view does not use the expected index.html template.{FAILURE_FOOTER}")
    ...
    def test_about_link_present(self):
        ...
        """
        expected = "<a href=\"/tradebooks/about/\">About</a><br />"
...
class StaticMediaTests(TestCase):
    ...
    Also tests for the two required files -- books.jpg and ddd.jpg.
    """
    def setUp(self):
        self.project_base_dir = os.getcwd()
    ...
    def test_does_static_directory_exist(self):
        ...
        Also checks for the presence of books.jpg in the images subdirectory.
        """
        ...
        does_books_jpg_exist = os.path.isfile(os.path.join(self.static_dir, 'images', 'books.jpg'))
        ...
        self.assertTrue(does_books_jpg_exist, f"{FAILURE_HEADER}We couldn't locate the books.jpg image in the /static/images/ directory. If you think you've included the file, make sure to check the file extension. Sometimes, a JPG can have the extension .jpeg. Be careful! It must be .jpg for this test.{FAILURE_FOOTER}")
    
    def test_does_media_directory_exist(self):
        """
        Tests whether the media directory exists in the correct location.
        Also checks for the presence of profile.jpg.
        """
        does_media_dir_exist = os.path.isdir(self.media_dir)
        does_profile_jpg_exist = os.path.isfile(os.path.join(self.media_dir, 'ddd.jpg'))
        ...
        self.assertTrue(does_profile_jpg_exist, f"{FAILURE_HEADER}We couldn't find the ddd.jpg image in /media/. Check the file extension; this is a common pitfall. It should .jpg. Not .png, .gif, or .jpeg!{FAILURE_FOOTER}")
...
class ExtraTests(TestCase):
    ...
    def setUp(self):
        self.project_base_dir = os.getcwd()
        self.template_dir = os.path.join(self.project_base_dir, 'templates', 'tradebooks')
        self.about_response = self.client.get(reverse('tradebooks:about'))
    ...
    def test_about_uses_template(self):
        ...
        Checks whether the index view uses a template -- and the correct one!
        """
        self.assertTemplateUsed(self.about_response, 'tradebooks/about.html', f"{FAILURE_HEADER}The about() view does not use the about.html template.{FAILURE_FOOTER}")
    ...
    def test_about_contains_required_text(self):
        ...
        for required_str in required:
            self.assertTrue(required_str in self.about_response.content.decode(), f"{FAILURE_HEADER}The expected string '{required_str}' was not found in the rendered /tradebooks/about/ response.{FAILURE_FOOTER}")
    
    def test_about_contains_profile(self):
        """
        Checks whether the rendered about view has the picture of Profile.
        """
        required_str = f"<img src=\"{settings.STATIC_URL}images/ddd.jpg\" alt=\"Picture of Profile\" />"
        self.assertTrue(required_str in self.about_response.content.decode(), f"{FAILURE_HEADER}The HTML markup to include the image of Profile in the about template was not found. It needs to match exactly what we are looking for. Check the book.{FAILURE_FOOTER}")
